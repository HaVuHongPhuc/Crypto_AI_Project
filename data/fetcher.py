...
class MarketDataFetcher:
  ...
  def fetch_historical_ohlcv(
      self, symbol: str, timeframe: str, total_candles: int = 3000
  ) -> pd.DataFrame:
    ...
    while len(all_ohlcv) < total_candles:
      ...
      candles = self.exchange.fetch_ohlcv(
          symbol, timeframe, since=since, limit=fetch_limit
      )
      if not candles:
        break
      ...
      since = (
          candles[-1][0] + timeframe_ms
      )  # dời mốc since ngay sau nến cuối vừa nhận
  ...
  # --- Các Alias hỗ trợ tương thích đa tầng (test script & controller) ---
  def get_historical_klines(
      self, symbol: str, timeframe: str = "5m", limit: int = 200
  ) -> pd.DataFrame:
    return self.fetch_ohlcv(symbol=symbol, timeframe=timeframe, limit=limit)
